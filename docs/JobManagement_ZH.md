...
处理快照工具基于 `events_{timestamp}.jsonl`（带时间戳的事件日志）和 DAG 结构提供 Data-Juicer 作业处理状态的全面分析。
...
- **256MB 目标**：优化分区以目标 256MB 每个分区（可通过 `partition.target_size_mb` 配置）
...
│   ├── events_{timestamp}.jsonl  # 事件日志（带时间戳的 JSONL 格式）
│   ├── logs/                 # 日志目录
...
# 分析事件（查找最新的带时间戳的事件文件）
python -c "from pathlib import Path; import json; job_dir = Path('/path/to/job'); events_file = max(job_dir.glob('events_*.jsonl'), key=lambda f: f.stat().st_mtime); events = [json.loads(line) for line in open(events_file)]; print(f'总事件数: {len(events)}')"
